...
```
2.2. Loading the near and far offset amplitudes **.add_near(), .add_far()**:.
    * Parameter:
...
```
2.3. Loading the horizon **.add_horizon()**:
    * Parameter:
...
```
3.2. Input generation via __\_\_call\_\___  method:
    * Parameters:
...
4.2.1 Perform model analysis/training/dimensionality reduction via **.reduce()** method:
...
4.2.2 Two dimension UMAP embedding
