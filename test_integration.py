import pytest
from SeismicReduction import DataHolder, Processor, UmapModel, VaeModel, set_seed, plot_agent
import numpy as np
...
def test_base_case_UMAP():
    ...
    assert((np.isclose(UMAP_base_case, umap.two_dimensions, atol=0.1)).all()), \
...
def test_base_case_VAE():
    ...
    # process data for input
    processor = Processor(dataholder)
    input2 = processor(flatten=[True, 12, 52], normalise=True)

    # run the VAE model on input
    vae = VaeModel(input2)
    vae.reduce(epochs=5, hidden_size=2, lr=1e-2, plot_loss=False)
    vae.to_2d(umap_neighbours=50, umap_dist=0.001)

    # load base case
    vae_base_case = np.load('test_cases/Flat-top12,bottom52,VAE-epochs5,latent2,lr0.01.npy')

    # Base case testing:
    assert (vae_base_case.shape == vae.two_dimensions.shape), \
        'Output embedding array shape does not match base case embedding array shape.'

    assert((np.isclose(vae_base_case, vae.two_dimensions, atol=0.1)).all()), \
        'The implementation does not match the base case for: \n' \
        'Flattened input : above 12, below 52, \n' \
